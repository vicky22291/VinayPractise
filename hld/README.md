...
| C3 Orchestration | [`concepts/temporal-durable-execution.md`](../concepts/temporal-durable-execution.md) | Durable execution: every activity result, timer, and signal is appended to a per-workflow event history in one txn with an outbox-style transfer task, and a fresh worker replays the code against it so finished steps never re-run. Workflow code must be deterministic (time, random, deploys are the traps), activities stay at-least-once so idempotency keys remain. Sticky cache avoids full replay, 5 s fallback. Ceiling is state transitions per second on persistence, shard count fixed at creation, 51,200 events / 50 MB per run so Continue-As-New. If the engine is the question, sections 3 to 6 are the design. | #6 scheduler, #11 provisioning, #25 booking, #29 payroll, #31 workflow engine, #32 integration platform, Uber / YouTube / flash sale |
...
|---|---|---|---|---|---|
| 1 | Book seller broker: query N sellers async, aggregate, return best price | [`book-seller-broker/`](book-seller-broker/) | 22 | Fan-out / fan-in, timeouts, late responses, cancellation, per-seller QPS, slow-seller isolation, broker crash mid-request | attempted + studied |
...
| 13 | Google Drive / Dropbox file sync | [`dropbox/`](dropbox/) | reported (also Google, Dropbox) | Chunking + dedup, delta sync, conflict resolution, metadata DB sharding | attempted |
...
|---|---|---|---|
| 1 | Book seller broker | attempted + studied | [my-attempt](book-seller-broker/my-attempt.md) · [README](book-seller-broker/README.md) · [solution](book-seller-broker/solution.md) · [diagrams](book-seller-broker/diagrams.md) · [edge-cases](book-seller-broker/edge-cases.md) · deep-dives: [deadlines-and-aggregation](book-seller-broker/deep-dives/deadlines-and-aggregation.md), [seller-limits-and-scale](book-seller-broker/deep-dives/seller-limits-and-scale.md), [crash-recovery](book-seller-broker/deep-dives/crash-recovery.md) · research: [sources](book-seller-broker/research/sources.md) · missing: excalidraw |
...
| 13 | Google Drive / Dropbox file sync | attempted | [my-attempt](dropbox/my-attempt.md) (mock interview 2026-09-18) · missing: README, solution, diagrams, edge-cases, deep-dives, excalidraw |
