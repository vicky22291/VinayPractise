...
|---|---|---|---|---|---|
| 1 | Book seller broker: query N sellers async, aggregate, return best price | `book-seller-broker/` | 22 | Fan-out / fan-in, timeouts, late responses, cancellation, per-seller QPS, slow-seller isolation, broker crash mid-request | todo |
| 2 | Slack / large-scale messaging platform | `slack-messaging/` | 15 | Per-channel ordering, fan-out to large channels, presence, offline delivery, multi-device sync | todo |
| 3 | Strongly consistent distributed file system without S3 (GFS style) | `distributed-file-system/` | 8 | Metadata sharding, chunk replication, consensus, fencing, rename vs delete races, hot directories, DR with RPO/RTO | todo |
| 4 | In-memory KV store with write-ahead log | `kv-store-wal/` | 6 | fsync semantics, crash recovery, snapshots, compaction, concurrent readers vs writers | todo |
| 5 | Immutable distributed object store | `immutable-object-store/` | reported | Partitioning, erasure coding vs replication, rebalancing, metadata index, durability math | todo |
...
| 7 | Datacenter network throttling / hierarchical rate limiting | `network-throttling/` | reported | Distributed counters, local vs global limits, fairness, fail-open vs fail-closed | todo |
