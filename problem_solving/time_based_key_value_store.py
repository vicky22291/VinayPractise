...
class TimeMap:
    ...
    def get(self, key: str, timestamp: int) -> str:
        if key not in self.data:
            return ""
        pi = bisect.bisect_left(self.data[key].timestamps, timestamp)
        if pi == len(self.data[key].timestamps) or self.data[key].timestamps[pi] != timestamp:
            pi -= 1
        return self.data[key].values[pi]
