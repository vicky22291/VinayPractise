...
- Authors: By Nathan Bronson, Member of Technical Staff
...
**Summary:** The diagram shows corrupted x86-64 stack frames where inactive frames can be clobbered and return addresses overwritten.

**Components:**

- Corrupted values using corrupted stack memory
- Return address using stack return metadata
- Saved frame pointer `%rbp` using x86-64 frame-pointer state
- `%rsp` using the x86-64 stack pointer
- `FUNC X` using a function stack frame
- Callee-saved registers using calling-convention-preserved state
- Local variables using stack storage
- Live stack frames using active stack memory
- Inactive stack frames using the red zone
- Inactive stack frames clobbered by signal handler using signal-handler stack activity

**Flows:**

- `%rbp` -> return address: identifies the stack-frame return location
- `%rsp` -> active stack frame: identifies the current stack position
- `FUNC X` -> saved frame pointer `%rbp`: stores the caller frame reference
- `FUNC X` -> callee-saved registers and local variables: stores function state
- Signal handler -> inactive stack frames: clobbers inactive stack memory

**Numbers:** none

```mermaid
%% Shows corrupted stack frames, stack pointers, and inactive-frame clobbering
flowchart TD
    C[Corrupted values]
    R1[Return address]
    F1[Saved frame pointer percent rbp]
    S[Stack pointer percent rsp]
    X[Function X]
    R2[Return address]
    F2[Saved frame pointer percent rbp]
    L[Callee saved registers and local variables]
    A[Live stack frames]
    Z[Inactive stack frames red zone]
    H[Inactive stack frames clobbered by signal handler]
    ...
    F1 -->|links frame| R1
    F2 -->|links frame| R2
    A -->|becomes inactive| Z
    Z -->|can be clobbered by| H
    ...
    class C,R1,R2 critical
    class S,F1,F2 service
    class X,L service
    class A client
    class Z cache
    class H external
...
<sub>source image: https://images.ctfassets.net/kftzwdyauwt9/3eBjjDfMl3Ja4JE1V8skqx/99caec095d4669342d2a013f0b6befda/Corrupted-stack_crashes_light_desktop.svg</sub>
...
**Summary:** Crash rates are plotted across days for rocket clusters 1 through 7, distinguishing return-to-null and misaligned-stack crashes.

**Components:**

- Return-to-null crash series
- Misaligned-stack crash series
- Rocket cluster number axis
- Days axis
...
**Numbers:** 1, 2, 3, 4, 5, 6, 7, 0, 10, 20, 30, 40, 50, 60

```mermaid
%% Shows crash-rate observations by rocket cluster over days
flowchart LR
    A[Return to null]:::service
    B[Misaligned stack]:::critical
    C[Rocket cluster number]:::client
    D[Days]:::client
    E[Clusters 1 through 7]:::store
...
<sub>source image: https://images.ctfassets.net/kftzwdyauwt9/2Cu7fZUNQdMpqwby9xRFkE/1d171387051612f2f8fb5e352764a2a2/Crash_rate_by_cluster_light_desktop.svg</sub>
...
We turned to Fermi estimation. If the vulnerable window is on the order of 10−1010^{-10} seconds and SIGUSR2 arrives every 10−210^{-2} seconds of CPU time, then each exception cleanup handler or catch block has a roughly 10−810^{-8} probability of losing the race.
...
We also upstreamed a self-contained reproducer and a [fix](https://github.com/libunwind/libunwind/commit/a9b9293b286c14b9ed19db501aa347b46edd8a28) to GNU libunwind, and verified that the other unwinders don’t have a similar issue.
