...
**Summary:** The diagram shows how cache failures, expensive queries, and write spikes can overload PostgreSQL, causing timeouts and retry-driven load amplification.

**Components:**

- Cache layer failure - caching layer
- Expensive queries - query workload
- Write spike - write workload
- PostgreSQL overload - PostgreSQL database
- Requests become slow or time out - request processing
- More requests due to retries - retry traffic
- More requests further increase the load - load amplification

**Flows:**

- Cache layer failure -> PostgreSQL overload: cache misses increase database load
- Expensive queries -> PostgreSQL overload: expensive queries consume database resources
- Write spike -> PostgreSQL overload: increased write traffic
- PostgreSQL overload -> Requests become slow or time out: degraded query performance
- Requests become slow or time out -> More requests due to retries: clients retry failed or delayed requests
- More requests due to retries -> PostgreSQL overload: retries further increase database load

**Numbers:** none

```mermaid
%% Shows the vicious cycle of PostgreSQL overload under load
flowchart TD
    A[Cache layer failure] -->|cache misses| D[PostgreSQL overload]
    B[Expensive queries] -->|database work| D
    C[Write spike] -->|write traffic| D
    D -->|higher latency| E[Requests become slow or time out]
    E -->|retry traffic| F[More requests due to retries]
    F -.->|further increase the load| D
...
<sub>source image: https://images.ctfassets.net/kftzwdyauwt9/5dECJjynPkxFF2XABuh3bt/3b5562e1f879fd0afd2deffe9cf1e142/OAI_The_Vicious_Cycle_Under_Load__Light_Desktop_.svg</sub>
...
**Summary:** User requests reach a Kubernetes Service that load-balances traffic across multiple PgBouncer deployments, with each deployment serving a separate PostgreSQL read replica.
...
- Kubernetes Cluster
- Kubernetes Service
- Kubernetes Deployments running PgBouncer pods
- PgBouncer pods
...
- User Requests -> Kubernetes Service: database requests
- Kubernetes Service -> Kubernetes Deployment 1: load-balanced requests
- Kubernetes Service -> Kubernetes Deployment 2: load-balanced requests
- Kubernetes Deployment 1 -> Read Replica 1: proxied database traffic
- Kubernetes Deployment 2 -> Read Replica 2: proxied database traffic

**Numbers:** 1, 2

```mermaid
%% Shows PgBouncer deployments behind a Kubernetes Service for PostgreSQL read replicas
flowchart TD
    requests[USER REQUESTS]
    service[KUBERNETES SERVICE]
    deployment1[KUBERNETES DEPLOYMENT 1<br/>Multiple PgBouncer pods]
    deployment2[KUBERNETES DEPLOYMENT 2<br/>Multiple PgBouncer pods]
    replica1[READ REPLICA 1]
    replica2[READ REPLICA 2]

    requests -->|database requests| service
    service -->|load balanced requests| deployment1
    service -->|load balanced requests| deployment2
    deployment1 -->|proxied database traffic| replica1
    deployment2 -->|proxied database traffic| replica2
    ...
    class deployment1,deployment2 service
    class replica1,replica2 store
```

<sub>source image: https://images.ctfassets.net/kftzwdyauwt9/kttDE3EiZ6roTjpbUpqxX/dd9ceee15663caa6e37a83bedc8626f5/OAI_PgBouncer_as_PostgreSQL_Proxy__Light_Desktop_.svg</sub>
...
**Summary:** PostgreSQL cascading replication routes WAL from a primary through intermediate replicas to multiple read replicas.

**Components:**

- Primary using PostgreSQL
- Intermediate replica using PostgreSQL
- Read replica using PostgreSQL
- WAL replication stream

**Flows:**

- Primary -> Intermediate replica: WAL
- Intermediate replica -> Read replica: WAL

**Numbers:** none

```mermaid
%% PostgreSQL cascading replication with intermediate replicas relaying WAL
flowchart LR
    P[Primary]
    I1[Intermediate replica]
    I2[Intermediate replica]
    R1[Read replica]
    R2[Read replica]
    R3[Read replica]
    R4[Read replica]
...
<sub>source image: https://images.ctfassets.net/kftzwdyauwt9/T9UJV7cX2uI7qSOQwuv0h/daf02e23cb5d013d9afce95691ec7ec8/PostgreSQL_Cascading_Replication__Light_Desktop_.svg</sub>
