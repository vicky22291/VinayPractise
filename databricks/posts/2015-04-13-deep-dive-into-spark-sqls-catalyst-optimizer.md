...
- Source: https://www.databricks.com/blog/2015/04/13/deep-dive-into-spark-sqls-catalyst-optimizer.html
- Published: 2015-04-13
- Authors: Michael Armbrust, Yin Huai, Cheng Liang, Reynold Xin, Matei Zaharia
- Categories: engineering, open-source
- Images: 3 total, 3 extracted as architecture

Check out the [Why the Data Lakehouse is Your Next Data Warehouse](https://www.databricks.com/resources/ebook/bring-data-warehousing-data-lakes?itm_data=deepdivesparkcatalyst-blog-whylakehouseisnextdw ) ebook to discover the inner workings of the Databricks Lakehouse Platform

---

Spark SQL is one of the newest and most technically involved components of Spark. It powers both SQL queries and the new [DataFrame API](https://www.databricks.com/blog/2015/02/17/introducing-dataframes-in-spark-for-large-scale-data-science.html). At the core of Spark SQL is the [Catalyst optimizer](https://www.databricks.com/glossary/catalyst-optimizer), which leverages advanced programming language features (e.g. Scala's [pattern matching](https://docs.scala-lang.org/tour/pattern-matching.html) and [quasiquotes](https://docs.scala-lang.org/overviews/quasiquotes/intro.html)) in a novel way to build an extensible query optimizer.

We recently published a [paper](https://people.csail.mit.edu/matei/papers/2015/sigmod_spark_sql.pdf) on Spark SQL that will appear in [SIGMOD 2015](http://www.sigmod2015.org/) (co-authored with Davies Liu, Joseph K. Bradley, Xiangrui Meng, Tomer Kaftan, Michael J. Franklin, and Ali Ghodsi). In this blog post we are republishing a section in the paper that explains the internals of the Catalyst optimizer for broader consumption.
...
**Summary:** Expression tree representing `x+(1+2)` using Add, Attribute, and Literal nodes.

**Components:**

- Add root expression node
- Attribute x input attribute node
- Add nested expression node
- Literal 1 constant node
- Literal 2 constant node

**Flows:**

- Add root -> Attribute x: left expression
- Add root -> Add nested: right expression
- Add nested -> Literal 1: left operand
- Add nested -> Literal 2: right operand

**Numbers:** 1, 2

```mermaid
%% Expression tree for x plus 1 plus 2
flowchart TD
    A[Add root] -->|left expression| B[Attribute x]
    A -->|right expression| C[Add nested]
    C -->|left operand| D[Literal 1]
    C -->|right operand| E[Literal 2]
...
<sub>source image: https://www.databricks.com/wp-content/uploads/2015/04/Screen-Shot-2015-04-12-at-8.11.11-AM.png</sub>
...
**Summary:** Spark SQL’s Catalyst optimizer transforms a SQL query or DataFrame through analysis, logical optimization, physical planning, cost selection, and code generation into RDDs.

**Components:**

- SQL Query: Spark SQL parser input
- DataFrame: Spark SQL DataFrame API input
- Unresolved Logical Plan: Catalyst analysis input
- Catalog: Metadata used for resolution
- Logical Plan: Resolved Catalyst plan
- Optimized Logical Plan: Rule-optimized Catalyst plan
- Physical Plans: Candidate execution plans
- Cost Model: Plan cost comparison
- Selected Physical Plan: Chosen execution plan
- RDDs: Spark execution output

**Flows:**

- SQL Query -> Unresolved Logical Plan: parsed query structure
- DataFrame -> Unresolved Logical Plan: DataFrame logical representation
- Catalog -> Unresolved Logical Plan: schema and relation metadata
- Unresolved Logical Plan -> Logical Plan: resolved attributes and relations
- Logical Plan -> Optimized Logical Plan: rule-based logical optimization
- Optimized Logical Plan -> Physical Plans: candidate physical plans
- Physical Plans -> Cost Model: plans for cost comparison
- Cost Model -> Selected Physical Plan: lowest-cost plan selection
- Selected Physical Plan -> RDDs: generated execution code and RDD operations

**Numbers:** none

```mermaid
%% Shows the Spark SQL Catalyst optimizer flow from inputs to RDDs
flowchart LR
    Q[SQL Query] -->|parsed query structure| U[Unresolved Logical Plan]
    D[DataFrame] -->|logical representation| U
    C[Catalog] -->|schema and relation metadata| U
    U -->|resolved attributes and relations| L[Logical Plan]
    L -->|rule based optimization| O[Optimized Logical Plan]
    O -->|candidate physical plans| P[Physical Plans]
    P -->|plans for cost comparison| M[Cost Model]
    M -->|lowest cost plan| S[Selected Physical Plan]
    S -->|generated execution code| R[RDDs]
    ...
    class Q,D client
    class U,L,O,P,S service
    class C,R store
    class M decision
```

<sub>source image: https://www.databricks.com/wp-content/uploads/2015/04/Screen-Shot-2015-04-12-at-8.41.26-AM.png</sub>
...
In total, the logical optimization rules are [800 lines of code](https://github.com/apache/spark/blob/fedbfc7074dd6d38dc5301d66d1ca097bc2a21e0/sql/catalyst/src/main/scala/org/apache/spark/sql/catalyst/optimizer/Optimizer.scala).
...
**Summary:** Benchmark comparing runtime performance for interpreted, hand-written, and generated code.
...
```mermaid
%% Benchmark runtime comparison for three execution approaches
flowchart LR
    I[Interpreted 35 seconds]
    H[Hand-written 8 seconds]
    G[Generated 9 seconds]
...
<sub>source image: https://www.databricks.com/wp-content/uploads/2015/04/Screen-Shot-2015-04-12-at-8.45.27-AM-300x129.png</sub>
...
In total, Catalyst’s code generator is about [700 lines of code](https://github.com/apache/spark/blob/fedbfc7074dd6d38dc5301d66d1ca097bc2a21e0/sql/catalyst/src/main/scala/org/apache/spark/sql/catalyst/expressions/codegen/CodeGenerator.scala).

This blog post covered the internals of Spark SQL’s Catalyst optimizer. It’s novel, simple design has enabled the Spark community to rapidly prototype, implement, and extend the engine. You can read through rest of the [paper here](https://people.csail.mit.edu/matei/papers/2015/sigmod_spark_sql.pdf). If you are attending SIGMOD this year, please drop by our session!

You can also find more information about Spark SQL from the following:

- [Spark SQL and DataFrame Programming Guide](https://spark.apache.org/docs/latest/sql-programming-guide.html) from Apache Spark
- [Data Source API in Spark](https://www.slideshare.net/databricks/yin-huai-20150325meetupwithdemos) presentation by Yin Huai
- [Introducing DataFrames in Spark for Large Scale Data Science](https://www.slideshare.net/databricks/introducing-dataframes-in-spark-for-large-scale-data-science) by Reynold Xin
- [Beyond SQL: Speeding up Spark with DataFrames](https://www.slideshare.net/databricks/spark-sqlsse2015public) by Michael Armbrust
